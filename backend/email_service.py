...
"""
    ...
    def _extract_key_tasks(self, description: str) -> str:
        ...
        return " • ".join(keywords[:3])  # Limit to 3 key tasks
    ...
    def generate_outreach_for_job(self, job_id: int, firm_contact: Optional[str] = None) -> int:
        ...
        try:
            ...
            agent_match = db.query(AgentMatch).filter(
                AgentMatch.job_id == job_id
            ).order_by(AgentMatch.confidence_score.desc()).first()
...
def generate_outreach_for_all_high_confidence_jobs(min_confidence: float = 0.8) -> List[int]:
    ...
    try:
        ...
        high_confidence_matches = db.query(AgentMatch).filter(
            AgentMatch.confidence_score >= min_confidence
        ).all()
        
        for match in high_confidence_matches:
            # Check if outreach already exists for this job
            existing_outreach = db.query(Outreach).filter(
                Outreach.job_id == match.job_id
            ).first()
